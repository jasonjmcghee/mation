 ...
 * Converted to TypeScript by Claude.
 *
 * Version 2012-03-09
 *
...
const noise: NoiseModule = (function() {
  const module: NoiseModule = {
    // @ts-ignore
    seed: (seed: number): void => {},
    // @ts-ignore
    simplex2: (xin: number, yin: number): number => 0,
    // @ts-ignore
    simplex3: (xin: number, yin: number, zin: number): number => 0
  };
  ...
  module.seed = function(seed: number): void {
    if (seed > 0 && seed < 1) {
      // Scale the seed out
      seed *= 65536;
    }
  ...
  // 2D simplex noise
  module.simplex2 = function(xin: number, yin: number): number {
  ...
  // 3D simplex noise
  module.simplex3 = function(xin: number, yin: number, zin: number): number {
